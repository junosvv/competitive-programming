...
signed main() {
    ...
    const int BIG = 2505;
    vector<vector<vt3i>> nodes(n, vector<vt3i>(BIG));
    vvi cost(n, vi(n)), time(n, vi(n));
    FOR(i, 0, m) {
        int x, y, a, b;
        cin >> x >> y >> a >> b;
        --x; --y;
        ...
        FOR(money, a, BIG) {
            nodes[x][money].emplace_back(y, money-a, b);
            nodes[y][money].emplace_back(x, money-a, b);
        }
    }
    FOR(i, 0, n) {
        int cn, tn;
        ...
        FOR(money, 0, BIG) {
            nodes[i][money].emplace_back(i, min(money+cn, BIG-1), tn);
        }
    }

    vvi dist(n, vi(BIG, INF));
    mini(s, BIG-1);
    dist[0][s] = 0;
    priority_queue<t3i, vt3i, greater<t3i>> q;
    q.emplace(0, 0, s);
    while (!q.empty()) {
        int du, u, money;
        tie(du, u, money) = q.top();
        q.pop();

        if (du != dist[u][money]) continue;

        for (t3i nxt : nodes[u][money]) {
            int v, newmoney, cost;
            tie(v, newmoney, cost) = nxt;

            if (mini(dist[v][newmoney], dist[u][money] + cost)) {
                q.emplace(dist[v][newmoney], v, newmoney);
            }
        }
    }
