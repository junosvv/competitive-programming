...
void f(string &s) {
    ...
    FOR(i, 0, n) {
        if ((n-1-i-1<0 || s[n-1-i] != s[n-1-i-1] + 1) && s[n-1-i] != '9') {
            ++s[n-1-i];
            FOR(j, n-1-i+1, n) s[j] = s[j-1]=='0' ? '0' : s[j-1] - 1;
            return;
        }
    }
    s = "1";
...
signed main() {
    ...
    if (c == 0) {
        int cnt = 0;
        FOR(i, 0, n) if (a[i]) ++cnt;
        if (cnt <= k) {
            FOR(i, 0, n) if (a[i]) print(i+1);
        }
    } else {
        ...
        FOR(i, 1, n) {
            prefdp[i] = prefdp[i-1];
            if (a[i]) maxi(prefdp[i], i-1-c >= 0 ? 1 + prefdp[i-1-c] : 1);
        }
        ROF(i, n-2, -1) {
            suffdp[i] = suffdp[i+1];
            if (a[i]) maxi(suffdp[i], i+1+c < n ? 1 + suffdp[i+1+c] : 1);
        }

        vi res(n+1);
        FOR(l, 1-c, n) {
            int r = l + c - 1;
            int lmax = l-1 >= 0 ? prefdp[l-1] : 0;
            int rmax = r+1 <= n-1 ? suffdp[r+1] : 0;
            if (lmax + rmax >= k) {
                ++res[max(0LL, l)];
                --res[min(r+1, n)];
            }
