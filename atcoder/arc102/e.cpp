...
main() {
    ...
    FOR(i, 2, k+2) {
        int h = (i-1) / 2;
        FOR(j, 0, h + 1) {
            res[i] = (res[i] + nCr(h,j) * powa(2, j) % MOD * nCr(n+k-i,k-i+j) % MOD) % MOD;
            if (!(i&1)) res[i] = (res[i] + nCr(h,j) * powa(2, j) % MOD * nCr(n+k-i-1,k-i+j) % MOD) % MOD;
        }
        res[2*k+2-i] = res[i];
    }
    FOR(i, 2, 2*k+1) print(res[i]);
}