...
main() {
    ...
    int BIG = 1000010;
    vi sieve(BIG);
    REP(p, 2, BIG) if (!sieve[p]) for (int u=p*p; u<BIG; u+=p) sieve[u] = p;
    ...
    REP(i, 0, n) {
        ...
        ++facts[x];
        for (auto pr : facts) {
            lcm[pr.first] = max(lcm[pr.first], pr.second);
        }
    }
    int res = 1;
    for (auto pr : lcm) res = res * powa(pr.first, pr.second) % MOD;
    int temp = 0;
    REP(i, 0, n) temp = (temp + inv(a[i])) % MOD;
    res = res * temp % MOD;
    print(res);
}