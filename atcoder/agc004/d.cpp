...
vvi invnodes;
vi dist;
vector<set<int>> bydist;
void dfs(int u) {
    for (int v : invnodes[u]) if (dist[v] == -1) {
        dist[v] = dist[u] + 1;
        bydist[dist[v]].insert(v);
        dfs(v);
    }
}
...
main() {
    ...
    ROF(x, n-1, k) {
        while (!bydist[x].empty()) {
            int u = *bydist[x].begin();
            FOR(i, 0, k-1) u = a[u];
