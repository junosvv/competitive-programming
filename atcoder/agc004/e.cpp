...
main() {
    ...
    if (er > (R-1)/2) {
        FOR(i, 0, R/2) FOR(j, 0, C) swap(grid[i][j], grid[R-1-i][j]);
        er = R-1-er;
    }
    if (ec > (C-1)/2) {
        FOR(i, 0, R) FOR(j, 0, C/2) swap(grid[i][j], grid[i][C-1-j]);
        ec = C-1-ec;
    }

    vvi pref = grid;
    FOR(r, 0, R) FOR(c, 1, C) pref[r][c] += pref[r][c-1];
    FOR(r, 1, R) FOR(c, 0, C) pref[r][c] += pref[r-1][c];

    auto sumq = [&](int r1, int c1, int r2, int c2) {
        int res = pref[r2][c2];
    ...
    int res = 0;
    FOR(sr, 0, er+1) FOR(sc, 0, ec+1) {
        vvi dp(R-er-er, vi(C-ec-ec));
        dp[0][0] = sumq(sr, sc, sr+er, sc+ec);
        FOR(i, 0, R-er-er) FOR(j, 0, C-ec-ec) {
            if (i) maxi(dp[i][j], dp[i-1][j] + sumq(sr+i+er, sc+j, sr+i+er, sc+j+ec));
            if (j) maxi(dp[i][j], dp[i][j-1] + sumq(sr+i, sc+j+ec, sr+i+er, sc+j+ec));
        }
        maxi(res, dp.back().back());
    }
    print(res);
}