...
main() {
    ...
    a.pop_back();
    --n;
    FOR(i, 0, n) if (a[i] != a[n-1-i]) {
        print(-1);
        exit(0);
    }

    vi ones;
    ROF(i, (n+1)/2, 0) if (a[i-1]) ones.push_back(i);

    vpii edges = {{1, 2}};
    int x = 2;
    FOR(i, 1, SIZE(ones)) {
        FOR(u, x+1, x+1 + ones[i-1]-ones[i]) edges.emplace_back(x, u);
        x += ones[i-1] - ones[i];
    }
    FOR(u, x+1, n+2) edges.emplace_back(1, u);
    for (pii pr : edges) print(pr.first, pr.second);
}