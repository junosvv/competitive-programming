...
main() {
    ...
    vector<mii> occ(2);
    FOR(i, 0, n) ++occ[i&1][a[i]];
 
 
    vector<vpii> pairs(2);
    FOR(d, 0, 2) {
        for (pii pr : occ[d]) {
            pairs[d].emplace_back(pr.second, pr.first);
        }
        pairs[d].emplace_back(0, -1);
        sort(RALL(pairs[d]));
    }
 
    if (pairs[0][0].second != pairs[1][0].second) {
        print(n - pairs[0][0].first - pairs[1][0].first);
    } else {
        print(n - max(pairs[0][0].first + pairs[1][1].first, pairs[0][1].first + pairs[1][0].first));
    }
}