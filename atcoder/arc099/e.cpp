...
vi par;
vvi cnt;
void dfs(int rep, int u, bool c=0) {
    if (seen[u]) {
        if (col[u] != c) {
            print(-1);
            exit(0);
...
main() {
    ...
    adj.assign(n, vb(n, true));
    FOR(i, 0, n) adj[i][i] = false;
    ...
    FOR(i, 0, n) {
        ...
        FOR(x, 0, 2) {
            FOR(j, 0, n) if (dp[j]) {
                if (j+cnt[i][0] < n) dp2[j+cnt[i][0]] = true;
                if (j+cnt[i][1] < n) dp2[j+cnt[i][1]] = true;
            }
        }
        dp = dp2;
    }
    ROF(i, n/2, -1) if (dp[i]) {
        print(i*(i-1)/2 + (n-i)*(n-i-1)/2);
        exit(0);
    }
}