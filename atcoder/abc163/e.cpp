...
signed main() {
    ...
    FOR(a, 0, n+1) FOR(b, 0, n+1) {
        if (a) {
            int happy = abs(arr[a+b-1].second-(a-1)) * arr[a+b-1].first;
            maxi(dp[a][b], happy + dp[a-1][b]);
        }
        if (b) {
            int happy = abs(arr[a+b-1].second-(n-b)) * arr[a+b-1].first;
            maxi(dp[a][b], happy + dp[a][b-1]);
        }
