...
signed main() {
    ...
    FOR(i, 0, n) {
        int a, b, c;
        cin >> a >> b >> c;
        circles[i] = make_tuple(a, b, c);
    }
    circles.emplace_back(temp[0], temp[1], 0);
    circles.emplace_back(temp[2], temp[3], 0);
    n += 2;
 
    vector<vector<dub>> adj(n, vector<dub>(n));
    FOR(i, 0, n) FOR(j, 0, n) {
        int dx = get<0>(circles[i]) - get<0>(circles[j]);
        int dy = get<1>(circles[i]) - get<1>(circles[j]);
        adj[i][j] = adj[j][i] = max(0.0L, sqrtl(dx*dx + dy*dy) - get<2>(circles[i]) - get<2>(circles[j]));
    }
 
    vector<dub> dist(n, INF);
    dist[n-2] = 0;
	vb proc(n);
	FOR(i, 0, n) {
		int u = -1;
		FOR(j, 0, n) if (!proc[j] && (u == -1 || dist[j] < dist[u])) u = j;
		
		proc[u] = true;
		FOR(v, 0, n) dist[v] = min(dist[v], dist[u] + adj[u][v]);
	}
	
    cout << setprecision(10) << dist.back();
}