...
main() {
    ...
    FOR(mask, 0, 1<<(R-1)) {
        ...
        FOR(r, 1, R) {
            trans[r] = r;
            while (trans[r] && !(mask>>trans[r]-1&1LL)) --trans[r];
        }
        bool failed = false;
        vb overf(C);
        FOR(c, 0, C) {
            FOR(r, 0, R) {
                if (white[r][c]) ++numw[trans[r]];
            }
            FOR(r, 0, R) if (numw[r] > K) overf[c] = true;
            if (overf[c]) {
                numw.assign(R, 0);
                FOR(r, 0, R) {
                    if (white[r][c]) ++numw[trans[r]];
                }
                FOR(r, 0, R) if (numw[r] > K) {
                    failed = true;
                    break;
                }
                if (failed) break;
