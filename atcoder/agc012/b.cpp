...
main() {
    ...
    FOR(i, 0, m) {
        ...
        FOR(j, 1, 11) {
            nodes[j*n+a].push_back((j-1)*n+b);
            nodes[j*n+b].push_back((j-1)*n+a);
        }
    }
    ...
    FOR(t, 0, Q) {
        int cent, d, col;
        tie(cent, d, col) = queries[t];
        --cent;

        if (res[d*n+cent] != INF) continue;
        res[d*n+cent] = t;
        ...
        while (!q.empty()) {
            ...
            for (int v : nodes[u]) if (res[v] == INF) {
                res[v] = t;
                q.push(v);
            }
        }
    ...
    FOR(u, 0, n) {
        int cur = INF;
        FOR(d, 0, 11) mini(cur, res[d*n+u]);
        if (cur == INF) print(0);
        else print(get<2>(queries[cur]));
    }
}