...
signed main() {
    ...
    vi bad(n);
    FOR(i, 1, n) bad[i] = a[i] - a[i-1] < x[0];
    vi badpref = bad;
    FOR(i, 1, n) badpref[i] += badpref[i-1];

    auto qbad = [&](int l, int r) {
        if (l > r) return 0LL;
        return badpref[r] - (l ? badpref[l-1] : 0);
    };

    vi nxt(n);
    int j = 0;
    FOR(i, 0, n) {
        while (j < n && a[j] - a[i] < x[1]) ++j;
        nxt[i] = j;
    }
    ...
    ROF(i, n-2, -1) {
        ...
        if (a[i+1] - a[i] < x[1]) {
            if (!qbad(i+2, nxt[i]-1)) {
                if (nxt[i] == n) {
                    dp[i][1] += 1;
                } else if (a[nxt[i]] - a[nxt[i]-1] < x[0]) {
                    if (nxt[i]+1 >= n || a[nxt[i]+1] - a[nxt[i]-1] >= x[0]) {
                        dp[i][1] += dp[nxt[i]][1];
                    }
                } else {
                    dp[i][1] += dp[nxt[i]][0] + dp[nxt[i]][1];
                }
            }
        } else {
