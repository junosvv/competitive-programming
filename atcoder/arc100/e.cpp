...
main() {
    ...
    vi order(p-1);
    FOR(i, 0, p-1) order[i] = i+1;
    sort(ALL(order), [](int x, int y) { return __builtin_popcount(x) < __builtin_popcount(y); });

    auto upd = [&](int &b, int &b2, int x) {
        if (x == -1 || x == b || x == b2) return;
        if (b == -1 || a[x] > a[b]) {
            b2 = b;
            b = x;
        } else if (b2 == -1 || a[x] > a[b2]) {
            b2 = x;
        }
    };
    ...
    for (int i : order) {
        ...
        FOR(d, 0, n) if (i>>d&1) {
            int x = i ^ 1<<d;
            upd(big[i], big2[i], big[x]);
            upd(big[i], big2[i], big2[x]);
        }
    }

    vi res(p);
    FOR(i, 1, p) res[i] = a[big[i]] + a[big2[i]];
    FOR(i, 1, p) maxi(res[i], res[i-1]);
    FOR(i, 1, p) print(res[i]);
}