...
signed main() {
    ...
    FOR(l, 1, BIG) {
        ROF(i, n-1, -1) {
            if (a[i] == l) nxt[l][i] = i;
            else if (i == n-1) nxt[l][i] = n;
            else nxt[l][i] = nxt[l][i+1];
        }
    }

    vector<vector<vvi>> dp(n, vector<vvi>(n, vvi(BIG, vi(BIG))));
    FOR(l, 1, BIG) {
        FOR(di, 0, n) {
            FOR(i, 0, n) {
                int j = i+di;
                if (j >= n) break;
                FOR(k, 1, l+1) {
                    if (k < l) maxi(dp[i][j][k][l], dp[i][j][k][l-1]);
                    int m = nxt[l][i];
                    while (m <= j) {
                        FOR(p, k, l+1) {
                            int u = i <= m-1 ? dp[i][m-1][k][p] : 0;
                            int v = m+1 <= j ? dp[m+1][j][p][l] : 0;
                            maxi(dp[i][j][k][l], 1 + u + v);
                        }
                        if (m+1 >= n) break;
                        m = nxt[l][m+1];
                    }
