...
signed main() {
    ...
    CASET {
        ...
        FOR(r, 0, n) FOR(c, 0, n) if (grid[r][c] != '.') {
            ++cnt[grid[r][c] == 'X'][(r+c) % 3];
            ++tot[(r+c) % 3];
        }
        int a = min_element(ALL(tot)) - tot.begin();
        int b = (a+1) % 3;
        if (tot[(b+1) % 3] < tot[b]) b = (b+1) % 3;
        char ta = cnt[0][a] + cnt[1][b] <= cnt[0][b] + cnt[1][a] ? 'X' : 'O';
        char tb = ta == 'X' ? 'O' : 'X';
        FOR(r, 0, n) FOR(c, 0, n) if (grid[r][c] != '.') {
            if ((r+c) % 3 == a) grid[r][c] = ta;
            else if ((r+c) % 3 == b) grid[r][c] = tb;
        }
        FOR(r, 0, n) {
            print(grid[r]);
        }   
