...
signed main() {
    ...
    int n, pistol, laser, awp, port;
    cin >> n >> pistol >> laser >> awp >> port;

    vi a(n);
    cin >> a;

    int themin = min(pistol, min(laser, awp));
    ...
    // print(n, ":", dp[n]);
    ROF(k, n-1, -1) {
        dp[k][0] = min(laser + port + dp[k+1][1], min(min(pistol, awp)*a[k] + awp + (k==n-1?0:port + dp[k+1][0]), min(pistol, awp)*a[k] + pistol + port + dp[k+1][1]));
        mini(dp[k][1], port + themin + port + dp[k][0]);
        mini(dp[k][1], laser + port + themin + port + themin + (k==n-1?0:port + dp[k+1][0]));
        mini(dp[k][1], laser + port + themin + port + port + dp[k+1][1]);
        mini(dp[k][1], min(pistol, awp)*a[k] + awp + port + themin + (k==n-1?0:port + port + dp[k+1][0]));
        mini(dp[k][1], min(pistol, awp)*a[k] + pistol + port + themin + port + themin + (k==n-1?0:port + dp[k+1][0]));
        // print(k, ":", dp[k]);
    }
    print(dp[0][0]);
}
