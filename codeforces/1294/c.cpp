...
main() {
    ...
    CASET {
        ...
        if (facts.size() == 1) {
            pii pr = *facts.begin();
            if (pr.second < 6) print("NO");
            else {
                print("YES");
                cout << pr.first << ' ' << sq(pr.first) << ' ' << powa(pr.first, pr.second-3) << '\n';
            }
        } else if (facts.size() == 2) {
            pii pr1 = *facts.begin();
            pii pr2 = *facts.rbegin();
            if (pr1.second == 1) {
                if (pr2.second <= 2) {
                    print("NO");
                } else {
                    print("YES");
                    cout << pr1.first << ' ' << pr2.first << ' ' << powa(pr2.first, pr2.second-1) << '\n';
                }
            } else if (pr2.second == 1) {
                if (pr1.second <= 2) {
                    print("NO");
                } else {
                    print("YES");
                    cout << pr2.first << ' ' << pr1.first << ' ' << powa(pr1.first, pr1.second-1) << '\n';
                }
            } else {
                print("YES");
                cout << pr1.first*pr2.first << ' ' << powa(pr1.first, pr1.second-1) << ' ' << powa(pr2.first, pr2.second-1) << '\n';
            }
        } else {
            pii pr1 = *facts.begin();
            pii pr2 = *facts.rbegin();
