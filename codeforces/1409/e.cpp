...
signed main() {
    ...
    CASET {
        ...
        int j = 0;
        vi toRight(n), toLeft(n);
        FOR(i, 0, n) {
            while (j<SIZE(x) && x[j] - x[i] <= k) ++j;
            toRight[i] = j-i;
        }
        int i = n-1;
        ROF(j, n-1, -1) {
            while (i>=0 && x[j] - x[i] <= k) --i;
            toLeft[j] = j-i;
        }
        vi pref(n), suff(n);
        pref[0] = toLeft[0];
        FOR(i, 1, n) {
            pref[i] = max(pref[i-1], toLeft[i]);
        }
        suff[n-1] = toRight[n-1];
        ROF(i, n-2, -1) {
            suff[i] = max(suff[i+1], toRight[i]);
        }

        // print(toLeft, toRight, pref, suff);

        int res = 1;
        FOR(i, 1, n) {
            maxi(res, pref[i-1] + suff[i]);
        }
        print(res);
    }
}
