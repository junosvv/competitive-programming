...
int solve(int u, int v) {
    if (u==v) dp[u][v] = 0;
    if (dp[u][v] == -1) dp[u][v] = max(solve(u, par[v][u]), solve(par[u][v], v)) + subsize[u][v]*subsize[v][u];
    return dp[u][v];
}
...
main() {
    ...
    cin >> n;
    nodes.resize(n);
    for (auto v : {&subsize, &par, &dp}) (*v).assign(n, vi(n, -1));
    REP(i, 0, n-1) {
        int a, b;
        cin >> a >> b;
        --a; --b;
    ...
    int res = 0;
    REP(x, 0, n) precalc(x, x);
    REP(i, 0, n) REP(j, 0, n) res = max(res, solve(i, j));
    print(res);
}
