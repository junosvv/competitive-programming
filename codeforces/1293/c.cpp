...
main() {
    ...
    REP(_, 0, q) {
        ...
        if (grid[r][c]) {
            if (r==0) {
                if (grid[r+1][c]) cols.insert(c);
                if (c-1>=0 && grid[r+1][c-1]) poss.insert(c-1);
                if (c+1<n && grid[r+1][c+1]) negs.insert(c);
            } else {
                if (grid[r-1][c]) cols.insert(c);
                if (c-1>=0 && grid[r-1][c-1]) negs.insert(c-1);
                if (c+1<n && grid[r-1][c+1]) poss.insert(c);
            }
        } else {
            if (r==0) {
                if (grid[r+1][c]) cols.erase(c);
                if (c-1>=0 && grid[r+1][c-1]) poss.erase(c-1);
                if (c+1<n && grid[r+1][c+1]) negs.erase(c);
            } else {
                if (grid[r-1][c]) cols.erase(c);
                if (c-1>=0 && grid[r-1][c-1]) negs.erase(c-1);
                if (c+1<n && grid[r-1][c+1]) poss.erase(c);
            }
        }
