...
signed main() {
    ...
    CASET {
        ...
        vvi grid(n, vi(n));
        cin >> grid;
        FOR(r, 0, n) FOR(c, 0, n) grid[r][c] -= 1;
        string s;
        cin >> s;
        ...
        vvi res(n, vi(n));
        FOR(r, 0, n) FOR(c, 0, n) {
            vi delta = {r, c, (grid[r][c]-grid[0][0]+n) % n};
            vi delta2(3);
            FOR(i, 0, 3) delta2[i] = delta[perm[i]];
            vi temp = pt;
            FOR(i, 0, 3) temp[i] = (temp[i] + delta2[i]) % n;
            res[temp[0]][temp[1]] = temp[2];
        }
        FOR(r, 0, n) {
            FOR(c, 0, n) cout << res[r][c] + 1 << ' ';
            print();
        }
        print();
    }
