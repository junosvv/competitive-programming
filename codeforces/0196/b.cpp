...
main() {
    ...
    deque<pii> q;
    vector<vpii> seen(R, vpii(C, {INF, INF}));
    REP(r, 0, R) REP(c, 0, C) if (grid[r][c] == 'S') {
        grid[r][c] = '.';
        q.emplace_back(r, c);
        seen[r][c] = {r, c};
    }
    ...
    while (!q.empty()) {
        int r, c; tie(r, c) = q.front();
        q.pop_front();
        
        for (pii delta : vector<pii>{{0, 1}, {1, 0}, {0, -1}, {-1, 0}}) {
            int rr = r+delta.first, cc=c+delta.second;
            int wr = (rr%R+R)%R, wc = (cc%C+C)%C;
            if (grid[wr][wc]=='#' || seen[wr][wc] == pii{rr, cc}) continue;
            if (seen[wr][wc] != pii{INF, INF}) {
                print("Yes");
                return 0;
            }
            seen[wr][wc] = {rr, cc};
