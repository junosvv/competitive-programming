...
signed main() {
    ...
    auto querymin = [&](int l, int r) {
        ...
        r += R;
        while (l <= r) {
            if (l&1) mini(res, treemin[l++]);
            if (!(r&1)) mini(res, treemin[r--]);
            l /= 2;
            r /= 2;
        }
        return res;
    ...
    auto querymax = [&](int l, int r) {
        ...
        r += R;
        while (l <= r) {
            if (l&1) maxi(res, treemax[l++]);
            if (!(r&1)) maxi(res, treemax[r--]);
            l /= 2;
            r /= 2;
        }
        return res;
    ...
    FOR(q, 0, Q) {
        ...
        if (querymax(er, R-1) > querymin(0, er)) {
            fail = true;
        }
        print(fail ? "NO" : "YES");
    }
