...
void dfs(int u, int p=-1) {
    ...
    for (int v : nodes[u]) if (v != p) {
        dfs(v, u);
        ++children;
        redc.push_back(red[v]);
        bothc.push_back(max(red[v], blu[v]));
        maxi(red[u], bothc.back());
        maxi(blu[u], redc.back());
    ...
    ++blu[u];
    red[u] += max(0LL, children-1);

    maxi(res, redc[0] + redc[1] + 1);
    maxi(res, bothc[0] + bothc[1] + max(0LL, children-2) + (p!=-1?1:0));
}
