...
signed main() {
    ...
    CASET {
        ...
        auto ispeak = [&](int x) {
            ...
            return abs((a[x] > a[x-1]) - (a[x] < a[x-1]) + (a[x] > a[x+1]) - (a[x] < a[x+1])) == 2;
        };
        ...
        FOR(i, 0, n) {
            FOR(k, 0, 2) {
                int oldpeaks = 0;
                FOR(j, i-1, i+2) {
                    if (ispeak(j)) oldpeaks += 1;
                }
                int orig = a[i];
                if (i+2*k-1 < 0 || i+2*k-1 >= n) continue;
                a[i] = a[i+2*k-1];
                int newpeaks = 0;
                FOR(j, i-1, i+2) {
                    if (ispeak(j)) newpeaks += 1;
                }
                mini(res, peaks + newpeaks - oldpeaks);
                a[i] = orig;
            }
        }
        print(res);
