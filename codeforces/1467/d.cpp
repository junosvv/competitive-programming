...
signed main() {
    ...
    FOR(x, 1, k+1) {
        FOR(i, 0, n) {
            if (x >= 2) num[i][x] = ((i?num[i-1][x-1]:0) + (i+1<n?num[i+1][x-1]:0)) % MOD;
            sum[i][x] = ((i?sum[i-1][x-1]:0) + (i+1<n?sum[i+1][x-1]:0) + a[i] * num[i][x]) % MOD;
        }
    }
    ...
    FOR(i, 0, n) {
        FOR(j, 1, k+1) {
            touches[i] = (touches[i] + num[i][j] * num[i][k+1-j]) % MOD;
        }
    }
    ...
    FOR(qi, 0, q) {
        ...
        int diff = (x - a[i] + MOD) % MOD;
        a[i] = x;

        res = (res + diff * touches[i]) % MOD;
        print(res);
    }
}
