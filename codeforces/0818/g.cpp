...
signed main() {
	...
	FOR(i, 0, n) {
		...
		addedge(3*n+i, i);
		if (last[a[i]] != -1) {
			addedge(3*n+last[a[i]], 3*n+i, 4);
		}
		if (last[a[i]-1] != -1) {
			addedge(n+last[a[i]-1], 3*n+i);
		}
		if (last[a[i]+1] != -1) {
			addedge(n+last[a[i]+1], 3*n+i);
		}
		if (lastm[a[i]%7] != -1) {
			addedge(2*n+lastm[a[i]%7], 2*n+i, 4);
			addedge(2*n+lastm[a[i]%7], i);
		}
		last[a[i]] = i;
		lastm[a[i]%7] = i;
	}
	...
	FOR(flow, 0, 4) {
		...
		FOR(p, 0, m) {
			...
			for (pii edge : edges) {
				int u, v;
				tie(u, v) = edge;
				if (!adj[u][v] || dist[u] == 1) continue;
				int c = 0;
				if (u < n && v == n+u) c = -1;
				else if (v < n && u == n+v) c = 1;
 
				if (mini(dist[v], dist[u] + c)) {
					something = true;
					par[v] = u;
				}
			}
