...
main() {
    ...
    cin >> n >> len >> k >> s;
    --k;
    map<char, int> occ, occ2;
    REP(i, 0, n*len) ++occ[s[i]];
    occ2 = occ;
    ...
    for (auto pr : occ) {
        REP(i, 0, pr.second) {
            words[i%(r-l)+l] += pr.first;
            --occ2[pr.first];
            if (occ2[pr.first] == 0) occ2.erase(pr.first);
            if (words[k].size() == len) {
                done = true;
                break;
            } 
    ...
    REP(w, 0, n) {
        REP(c, 0, len) {
            if (c == words[w].size()) {
                char nxt = (*occ2.begin()).first;
                words[w] += nxt;
                --occ2[nxt];
                if (occ2[nxt] == 0) occ2.erase(nxt);
            }
        }
        cout << words[w] << '\n';
    }
